...
def _load_prediction_rounds(
    ...
    for payload in payloads:
        ...
        for item in payload.get("predictions", []):
            ...
            round_name = item.get("round") or payload.get("round")
            if round_name:
    ...
    for round_name in sorted(round_meta, key=_round_sort_key):
        fixtures = sorted(
...
def _enrich_payload_round(payload: dict[str, Any]) -> list[dict[str, Any]]:
    predictions = [_enrich_match(item) for item in payload.get("predictions", [])]
    round_name = payload.get("round") or (
        predictions[0].get("round") if predictions else "Unknown"
    )
    return predictions
